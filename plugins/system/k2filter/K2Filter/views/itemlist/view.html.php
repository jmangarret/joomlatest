	...
	function display($tpl = null) {
		...
		switch ($task) {
			...
			case 'filter':
				...
				if($moduleParams->restmode == 1) { //auto restriction
				...
				switch ($filter_template) {
					...
					case 2 :
						...
						//multirate && multiimages compatibility
						if (JPluginHelper::isEnabled('system', 'k2multirate') || JPluginHelper::isEnabled('system', 'k2multiimages')) {
							...
							$images_plugin = JPluginHelper::getPlugin('system', 'k2multiimages');
							$images_plgParams = class_exists('JParameter') ? new JParameter($images_plugin->params) : new JRegistry($images_plugin->params);
							
							$template = $images_plgParams->get("template", "standart");
							
							if (JPluginHelper::isEnabled('system', 'k2multirate')) {
								$pluginPath = JPATH_SITE.DS.'plugins'.DS.'system'.DS.'k2multirate'.DS.'K2Multirate';
								$pluginPathSite = JURI::root().'plugins/system/k2multirate/K2Multirate';
							}
							else {
							...
							if (JPluginHelper::isEnabled('system', 'k2multiimages')) {						
								...
								$document->addStyleSheet($pluginPathSite.'/assets/highslide/highslide.css');	
										...
										$document->addScriptDeclaration("
											...
											if (typeof jQuery == 'undefined') {
												document.write('<scr'+'ipt type=\"text/javascript\" src=\"".JURI::root()."plugins/system/k2multiimages/K2MultiImages/assets/jquery-1.7.1.min.js\"></scr'+'ipt>');
												document.write('<scr'+'ipt>jQuery.noConflict();</scr'+'ipt>');
											}
											
											hs.graphicsDir = '".JURI::root()."plugins/system/k2multiimages/K2MultiImages/assets/highslide/graphics/';
								...
								if($template == "slider" || $template == "slider-hs") {
										
										$document->addStyleSheet(JURI::root().'plugins/system/k2multiimages/K2MultiImages/assets/slider/nivo-slider.css');
										$document->addStyleSheet(JURI::root().'plugins/system/k2multiimages/K2MultiImages/assets/slider/themes/default/default.css');
									
										$document->addScript(JURI::root().'plugins/system/k2multiimages/K2MultiImages/assets/slider/jquery.nivo.slider.pack.js');
							...
							if (JPluginHelper::isEnabled('system', 'k2multirate')) {
								...
								$rating_plugin = JPluginHelper::getPlugin('system', 'k2multirate');
								$rating_plgParams = class_exists('JParameter') ? new JParameter($rating_plugin->params) : new JRegistry($rating_plugin->params);
								
								$multirating_enabled = 1;
								
										if($rating_plgParams->get("catfilter", 0) == 1) {
											$selected = $rating_plgParams->get("category_id", 1);
											...
											foreach($catids as $catid) {
												if($rating_plgParams->get("getChildren", 1)) {
											...
											$moduleParams = modK2FilterHelper::getModuleParams(JRequest::getInt("moduleId"));
											$current_cat = JRequest::getVar('category');
											if(!is_array($current_cat)) {
												$current_cat = Array($current_cat);
											}
											if($current_cat) {
												$current_cat = explode(",", $moduleParams->restcat);
											}
											$multirating_enabled = 0;
											foreach($current_cat as $id) {
								...
								if(JPluginHelper::isEnabled('system', 'k2multiimages') && $images_plgParams->get("incategory", 0)) {
									switch($template) {						
										case "slider" :
											$this->setLayout('category-multirate-slider');
										break;								
											
										case "slider-hs" :
											$this->setLayout('category-multirate-slider-hs');
										break;			
											
										default :
											$this->setLayout('category-multirate-standard');
										break;										
									}									
								}
								else {
									if($multirating_enabled == 1) {
										$this->setLayout('category-multirate');
									}
								}
							}
							if(JPluginHelper::isEnabled('system', 'k2multiimages') && $images_plgParams->get("incategory", 0)) {
								switch($template) {						
									case "slider" :
										$this->setLayout('category-multiimages-slider');
									break;								
											
									case "slider-hs" :
										$this->setLayout('category-multiimages-slider-hs');
									break;			
											
									default :
										$this->setLayout('category-multiimages');
									break;										
								}
							}
						}
		...
		if (JPluginHelper::isEnabled('system', 'k2multiimages')) {
			...
			$version = new JVersion;
			$joomlaVersion = $version->RELEASE;
			$pluginPathImages = JPATH_SITE.DS.'plugins'.DS.'system'.DS.'k2multiimages'.DS.'K2MultiImages';
			$pluginPathSiteImages = JURI::root().'plugins/system/k2multiimages/K2MultiImages';
		
			$controller->addModelPath($pluginPathImages.DS."models");
