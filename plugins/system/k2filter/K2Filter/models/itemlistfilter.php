...
class K2ModelItemlistfilter extends K2Model {
		...
		function getData($ordering = NULL) {
							...
							$i = $matches[1];
								...
								if(!empty($search_from)) {
									...
									foreach($items as $item) {
										...
										preg_match("/\"{$i}\",\"value\":\"((\d+(?:(\.|\,)\d+)?)|(\d{4}-\d{2}-\d{2}))\"/", $extra, $match);
                               ...
                               if (!empty($search)) {
								...
								if(!empty($taga)) {							
									...
									if (JFile::exists(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_joomfish'.DS.'joomfish.php')) {
										...
										$db->setQuery($sql);
										$results = K2_JVERSION == '30' ? $db->loadColumn() : $db->loadResultArray();
									}
								...
								//item rating
								$rating_from = JRequest::getVar('rating-from');
								if($rating_from) {
									$query .= " AND (r.rating_sum/r.rating_count) >= {$rating_from}";	
								}
								
								$rating_to = JRequest::getVar('rating-to');
								if($rating_to) {
			...
			// ADDED k2FSM from here ->>
				...
				if($order == '') {
					$order = $moduleParams->ordering_default;
				}
				...
				$type = 'any';
                                ...
                                if($slider == 0 and $array == 0) {
                                                ...
                                                if ($n != 1) {
													...
													$search_upper = $json->encode(mb_strtoupper($search, "utf-8"));
													...
													$search_lower = $json->encode(mb_strtolower($search, "utf-8"));
													...
													$search_upper_first = $json->encode(mb_strtoupper(mb_substr($search, 0, 1, "utf-8")) . mb_strtolower(mb_substr($search, 1, mb_strlen($search), "utf-8")));
													...
													$search_upper_first_orig = $json->encode(mb_strtoupper(mb_substr($search, 0, 1, "utf-8")) . mb_substr($search, 1, mb_strlen($search)));
													...
													$search_upper_first_last = $json->encode(mb_strtoupper(mb_substr($search, 0, 1, "utf-8")) . mb_strtolower(mb_substr($search, 1, (mb_strlen($search)-2), "utf-8")) . mb_strtoupper(mb_substr($search, (mb_strlen($search)-1), 1, "utf-8")));
